...
const analystDialogueSchema=z.object({
  ...
  alternatives:z.array(z.string().max(360)).max(4),
...
async function continueAnalystDialogue(input:{brandId:string;understanding:NonNullable<OperatingState["productUnderstandings"][number]>;userMessage:string;mode:"ANSWER"|"HELP"}) {
  ...
  for(let attempt=1;attempt<=2;attempt+=1){
    try {
      const result=await generateText({model:analysisModel,system:expertiseSystem,prompt,providerOptions:{gateway:{user:executionWorkspaceId??"lafwiron-owner",tags:["feature:brand-analyst","mode:dry-run","budget:owner-approved-20-usd"],cacheControl:"s-maxage=3600"}},maxOutputTokens:1_600,abortSignal:AbortSignal.timeout(55_000)});
      cumulativeInputTokens+=result.usage.inputTokens??0;
      cumulativeOutputTokens+=result.usage.outputTokens??0;
