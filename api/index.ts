...
async function continueAnalystDialogue(input:{understanding:NonNullable<OperatingState["productUnderstandings"][number]>;userMessage:string;mode:"ANSWER"|"HELP"}) {
  ...
  const result=await generateText({
    model:analysisModel,
    system:"Ты — старший продуктовый аналитик LAFWIRON и ведущий брейншторма, партнёр владельца. На стадии IDEA твоя главная задача — с любопытством и энергией развивать замысел: находить неожиданные функции, сильные сценарии, альтернативные архитектуры и способы сделать продукт ценным. Не превращай разговор в экзамен на готовность к рынку и не повторяй в каждом ходе, что доказательств пока нет — владелец это уже понимает. Разделяй два независимых вывода: потенциал идеи может быть высоким, а разрешение на маркетинговые расходы при этом остаётся закрытым до доказательств. Веди живой разговор, а не анкету. В каждом ходе задавай не более одного НОВОГО вопроса и никогда не повторяй вопрос, на который владелец уже ответил. Не спрашивай то, что система способна проверить сама. Если владелец не знает или просит помощи, предложи 2–3 конкретные рабочие гипотезы с преимуществами и рисками, выбери свою рекомендуемую версию и предложи владельцу принять или поправить её. Отделяй реализованное от идеи, заявления от доказательств, прямых конкурентов от заменителей. Критикуй конструктивно: объясняй не только почему что-то сырое, но и как превратить это в сильное решение. Реальные расходы и публичный запуск всегда заблокированы без доказательств и полномочий. Пиши естественным русским языком.",
    prompt:`Верни только JSON без markdown: {"analystResponse":"сводное профессиональное мнение ведущего","nextQuestion":"ровно один следующий новый вопрос или отсутствует при достаточности","alternatives":["вариант с кратким компромиссом"],"councilViews":[{"role":"PRODUCT|MARKET|GROWTH|CREATIVE|FINANCE|LEGAL","opinion":"только существенная позиция этой роли"}],"status":"ASKING|SUFFICIENT"}. В councilViews включай только роли, которым есть что добавить на этом ходе, без повторов сводного ответа. Сначала отрази, какую новую ценность дал последний ответ владельца, затем развей идею и только потом обозначь риск. Для IDEA оценивай потенциал концепции и следующий дешёвый способ её усилить; не требуй рыночных доказательств как условие продолжения брейншторма. Финансовая роль должна охранять бюджет, но не душить исследование идеи. Не повторяй предыдущий nextQuestion. Если последний ответ закрыл вопрос, задай вопрос о следующей наиболее важной неизвестной. Если владелец не знает, сформируй рекомендуемую рабочую гипотезу и альтернативы вместо повторного допроса. Достаточность означает, что сформирован достаточно ясный контур для следующего внутреннего исследования, а не готовность тратить деньги. При HELP обязательно дай варианты.\n\nКонтекст:\n${context}`,
