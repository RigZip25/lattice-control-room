...
async function continueAnalystDialogue(input:{brandId:string;understanding:NonNullable<OperatingState["productUnderstandings"][number]>;userMessage:string;mode:"ANSWER"|"HELP"}) {
  ...
  const expertiseLead=input.understanding.founderExpertise?.trim()?`Совет принимает ваш профессиональный контекст как сильную исходную экспертизу: ${input.understanding.founderExpertise.trim()}. Поэтому мы не просим заново доказывать существование знакомой вам боли. `:"";
