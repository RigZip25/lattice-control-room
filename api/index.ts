...
async function continueAnalystDialogue(input:{brandId:string;understanding:NonNullable<OperatingState["productUnderstandings"][number]>;userMessage:string;mode:"START"|"ANSWER"|"HELP"}) {
  ...
  const transcript=(input.understanding.analystDialogue??[]).slice(-8).map((turn)=>({owner:turn.ownerMessage,analyst:turn.analystResponse,question:turn.nextQuestion,status:turn.status,readiness:turn.readiness}));
  ...
  const context=JSON.stringify({maturity:input.understanding.maturity??"MVP",ownerDescription:input.understanding.ownerDescription,founderExpertise:input.understanding.founderExpertise??"Не указана",analysis,transcript,userMessage:input.userMessage,mode:input.mode,discussion:{answeredQuestions:transcript.length,minimumCouncilRounds,nextCouncilRole,councilOrder,agenda:{PRODUCT:"целостность продукта, зрелость и первый работающий сценарий",MARKET:"боль, аудитория, конкуренты и готовность рынка",GROWTH:"спрос, каналы и механизм распространения",CREATIVE:"нарратив, исходные материалы и производимый контент",FINANCE:"модель заработка, экономика теста и предел риска",LEGAL:"правовые, платформенные и культурные ограничения"}}}).slice(0,28_000);
  ...
  for(let attempt=1;attempt<=2;attempt+=1){
    try {
      ...
      if(parsed.success&&(JSON.stringify(parsed.data).match(/[А-Яа-яЁё]/g)?.length??0)>=20) {
        ...
        const finalTurn=parsed.data.status==="SUFFICIENT"&&readinessClear&&transcript.length+1>=minimumCouncilRounds;
