...
export async function renderChoropleths(root = document) {
  ...
  await Promise.all(maps.map(async (container) => {
    ...
    const rankingLabel = mode === "states" ? `ВСЕ ШТАТЫ · ${listedFeatures.length}` : mode === "counties" ? "ТЕРРИТОРИИ С НАИБОЛЬШИМ СИГНАЛОМ" : "РЕГИОНЫ С НАИБОЛЬШИМ СИГНАЛОМ";
