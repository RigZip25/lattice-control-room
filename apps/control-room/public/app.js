...
function render() {
  ...
  document.getElementById("app").innerHTML = `
    ...
    <div class="stats-ribbon"><span>${state.brandProfiles.length} ${tr("БРЕНДОВ","BRANDS")} • ${state.productUnderstandings.filter((item)=>item.websiteResearch?.analysis).length} ${tr("ИЗУЧЕНО","REVIEWED")} • ${state.activationSprints.filter((item)=>item.status==="ACTIVE").length} ${tr("СПРИНТОВ","SPRINTS")} • ${state.addedMarkets.length} ${tr("РЫНКОВ","MARKETS")}</span><b>BUILD 2026.09.02.16 · DRY RUN / ${tr("ФАКТИЧЕСКОЕ СОСТОЯНИЕ","ACTUAL STATE")}</b></div>
    <div class="workspace">
...
document.addEventListener("click", async (event) => {
  ...
  try {
    ...
    if(target.dataset.action==="confirm-and-open-council"){
      const brandId=String(target.dataset.brandId);const understanding=state.productUnderstandings.find((item)=>item.brandId===brandId);
      if(understanding?.status!=="CONFIRMED")await sendCommand({kind:"CONFIRM_PRODUCT_UNDERSTANDING",brandId});
