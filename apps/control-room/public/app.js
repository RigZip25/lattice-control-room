...
function researchedProductMarkup(brand,understanding) {
  ...
  return `<article class="module understanding-review research-result"><div class="module-title">${tr("ПОНИМАНИЕ ПРОДУКТА","PRODUCT UNDERSTANDING")} <span>${tr("ЭТАП 02 · ПРОВЕРКА ВЛАДЕЛЬЦЕМ","STAGE 02 · OWNER REVIEW")}</span></div>${semantic}<div class="product-decision"><p>${tr("Проверьте только суть продукта. Исследование рынка, конкурентов и бюджета начнётся после подтверждения.","Confirm only the product understanding. Market, competitor, and budget research begins afterward.")}</p><div class="modal-actions"><button data-action="edit-brand" data-brand-id="${esc(brand.id)}">${tr("ИСПРАВИТЬ ИЛИ ДОПОЛНИТЬ","CORRECT OR ADD CONTEXT")}</button><button data-action="research-website" data-brand-id="${esc(brand.id)}">${tr("ПОВТОРИТЬ АНАЛИЗ","RUN ANALYSIS AGAIN")}</button><button class="primary" data-action="confirm-understanding" data-brand-id="${esc(brand.id)}">${tr("ПОДТВЕРДИТЬ ПОНИМАНИЕ","CONFIRM UNDERSTANDING")}</button></div></div></article>`;
}
