...
function expansionThesisControlMarkup(requestedBrandId) {
  ...
  return `<section class="module expansion-thesis"><div class="module-title">${tr("ТЕЗИС ЭКСПАНСИИ","EXPANSION THESIS")} <span>${thesis?"DRAFT · RANKED":"DIAGNOSIS CONFIRMED"}</span></div><header><div><h2>${tr("Сравнение географий до выделения бюджета","Compare geographies before budget allocation")}</h2><p>${tr("Рейтинг определяет порядок исследования, а не разрешение на запуск.","The ranking determines research order, not launch authority.")}</p></div>${thesis?"":`<button data-action="add-thesis" data-brand-id="${esc(brand.id)}">＋ ${tr("СОЗДАТЬ ТЕЗИС","CREATE THESIS")}</button>`}</header>${thesis?`<div class="thesis-ranking">${ranked.map((item,index)=>`<article><i>${String(index+1).padStart(2,"0")}</i><span><b>${esc(item.geographyName)}</b><small>${esc(item.countryCode)} · ${esc(item.administrativeLevel)}</small></span><strong>${score(item)}</strong><p>${esc(item.rationale)}</p><em>${tr("ПРОВЕРИТЬ","VALIDATE")}: ${esc(item.validationQuestions.join(" · "))}</em></article>`).join("")}</div>`:`<div class="thesis-empty"><b>${tr("Диагноз готов. Теперь выберите две географии для первого сравнения.","The diagnosis is ready. Choose two geographies for the first comparison.")}</b><span>${tr("Система сравнит спрос, доступность, конкурентную плотность и правовые ограничения. Это исследовательский приоритет, а не решение потратить деньги.","The system compares demand, accessibility, competitive density, and legal constraints. This is a research priority, not a decision to spend money.")}</span></div>`}</section>`;
}
...
function brandOnboardingMarkup() {
  ...
  const currentStage=thesis?5:diagnosis?.confirmedAt?4:councilComplete?3:2;
...
function render() {
  ...
  const onboardingBrand=["brand-onboarding","brand-diagnosis","brand-expansion"].includes(screen.key)?state.brandProfiles.find((item)=>item.id===decodeURIComponent(location.pathname.split("/")[2]??"")):null;
  ...
  document.getElementById("app").innerHTML = `
    ...
    <div class="stats-ribbon"><span>${state.brandProfiles.length} ${tr("БРЕНДОВ","BRANDS")} • ${state.productUnderstandings.filter((item)=>item.websiteResearch?.analysis).length} ${tr("ИЗУЧЕНО","REVIEWED")} • ${state.activationSprints.filter((item)=>item.status==="ACTIVE").length} ${tr("СПРИНТОВ","SPRINTS")} • ${state.addedMarkets.length} ${tr("РЫНКОВ","MARKETS")}</span><b>BUILD 2026.09.02.12 · DRY RUN / ${tr("ФАКТИЧЕСКОЕ СОСТОЯНИЕ","ACTUAL STATE")}</b></div>
    <div class="workspace">
