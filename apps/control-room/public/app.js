...
const state = { executive:false, locale:"RU", notice:"", decisions:3, selectedFilter:"ВСЕ", selectedRegion:"WORLD", welcome:location.pathname==="/", addCountry:false, addBrand:false, pendingCountry:null, pendingArea:null, addedMarkets:[], expansionAreas:[], brandProfiles:[], version:0 };
let screens = [];
let control = null;
let geographies = [];
let countryCatalog = [];
...
function welcomeMarkup() {
  return `<div class="welcome-backdrop"><section class="welcome-panel"><header><div class="welcome-brand"><strong>LATTICE</strong><small>MARKET FACTORY OS</small></div><span><i></i>${tr("ФАБРИКА ГОТОВА К АВТОНОМНОЙ РАБОТЕ","FACTORY READY FOR AUTONOMOUS OPERATION")}</span></header><div class="welcome-hero"><p>${tr("ДОБРО ПОЖАЛОВАТЬ В КОМАНДНЫЙ ЦЕНТР","WELCOME TO THE COMMAND CENTER")}</p><h1>${tr("Маркетинговая фабрика, которая работает круглосуточно.","A marketing factory that operates around the clock.")}</h1><p>${tr("Три восьмичасовые смены исследуют рынки, создают и проверяют контент, распределяют бюджет и обучаются на результатах. Исполнительные очереди не останавливаются во время пятиминутных агентных брейнштормов.","Three eight-hour shifts scout markets, create and review content, allocate budget and learn from outcomes. Execution queues continue during five-minute agent brainstorms.")}</p></div><div class="welcome-status"><article><small>${tr("ТЕКУЩАЯ СМЕНА","CURRENT SHIFT")}</small><b>SHIFT B</b><span>${tr("Производственный цикл","Production cycle")}</span></article><article><small>${tr("СЛЕДУЮЩИЙ БРЕЙНШТОРМ","NEXT BRAINSTORM")}</small><b>42 MIN</b><span>5 ${tr("минут · очереди продолжаются","minutes · queues continue")}</span></article><article><small>${tr("АВТОНОМНОСТЬ","AUTONOMY")}</small><b>GOVERNED</b><span>${tr("Исключения → владельцу","Exceptions → owner")}</span></article></div><div class="welcome-flow"><span>${tr("Исследование","Intelligence")}</span><i>→</i><span>${tr("Эксперименты","Experiments")}</span><i>→</i><span>${tr("Контент","Content")}</span><i>→</i><span>${tr("Дистрибуция","Distribution")}</span><i>→</i><span>${tr("Обучение","Learning")}</span><i>→</i><span>${tr("Капитал","Capital")}</span></div><footer><button data-action="welcome-factory">${tr("ПОСМОТРЕТЬ РАБОТУ ЦЕХОВ","VIEW FACTORY FLOOR")}</button><button class="primary" data-action="welcome-command">${tr("ВОЙТИ В КОМАНДНЫЙ ЦЕНТР","ENTER COMMAND CENTER")} →</button></footer></section></div>`;
}
