...
function brandOnboardingMarkup() {
  ...
  const diagnosis=state.productDiagnoses.find((item)=>item.brandId===brand.id);
  const thesis=state.expansionTheses.find((item)=>item.brandId===brand.id);
  const councilComplete=Boolean(understanding?.analystDialogue?.some((turn)=>turn.status==="SUFFICIENT"));
  const cycleReady=sources.length>=2&&facts>=3&&unknowns>=1&&Boolean(diagnosis&&thesis);
  const currentStage=thesis?5:diagnosis?4:councilComplete?3:2;
  ...
  let nextAction=councilComplete&&!diagnosis
    ? `<h3>${tr("Понимание продукта и рынка завершено","Product and market understanding is complete")}</h3><p>${tr("Совет зафиксировал профессиональный контекст владельца, основные боли, первый сценарий и границы неизвестного. Фабрика готова собрать рабочий диагноз продукта.","The council recorded founder expertise, core pains, the first scenario and remaining unknowns. The factory is ready to build the working product diagnosis.")}</p><button class="primary" data-route="/factory-config">${tr("ПЕРЕЙТИ К ДИАГНОЗУ ПРОДУКТА","CONTINUE TO PRODUCT DIAGNOSIS")} →</button>`
    : diagnosis&&!thesis
    ? `<h3>${tr("Диагноз продукта сформирован","Product diagnosis is complete")}</h3><p>${tr("Следующий этап — сравнить рынки и сформировать последовательность экспансии.","Next, compare markets and shape the expansion sequence.")}</p><button class="primary" data-route="/factory-config">${tr("СФОРМИРОВАТЬ ТЕЗИС ЭКСПАНСИИ","BUILD EXPANSION THESIS")} →</button>`
...
function render() {
  ...
  document.getElementById("app").innerHTML = `
    ...
    <div class="stats-ribbon"><span>${state.brandProfiles.length} ${tr("БРЕНДОВ","BRANDS")} • ${state.productUnderstandings.filter((item)=>item.websiteResearch?.analysis).length} ${tr("ИЗУЧЕНО","REVIEWED")} • ${state.activationSprints.filter((item)=>item.status==="ACTIVE").length} ${tr("СПРИНТОВ","SPRINTS")} • ${state.addedMarkets.length} ${tr("РЫНКОВ","MARKETS")}</span><b>BUILD 2026.08.28.8 · DRY RUN / ${tr("ФАКТИЧЕСКОЕ СОСТОЯНИЕ","ACTUAL STATE")}</b></div>
    <div class="workspace">
