...
function strategySurfaceMarkup(screen) {
  ...
  const rows = screen.key === "brands" ? (realBrandRows.length?realBrandRows:spec.rows.map((cells)=>({cells,route:"/brands"}))) : spec.rows.map((cells,index)=>({cells,route:byKey(screen.linksTo[index % screen.linksTo.length])?.route ?? screen.route}));
  return `<section class="strategy-surface"><header class="strategy-bar"><div><small>${esc(spec.eyebrow)}</small><b>${screen.key==="brands"&&state.brandProfiles.length?esc(tr(`${state.brandProfiles.length} БРЕНДОВ В СИСТЕМЕ`,`${state.brandProfiles.length} BRANDS IN SYSTEM`)):esc(spec.status)}</b></div><button data-route="${esc(byKey(spec.next)?.route ?? "/command")}">${esc(spec.action)} →</button></header><div class="strategy-stages">${spec.stages.map(([label,value,note],index)=>`<button data-route="${screen.route}"><i>${String(index+1).padStart(2,"0")}</i><span><small>${esc(label)}</small><b>${screen.key==="brands"&&index===0&&state.brandProfiles.length?state.brandProfiles.length:esc(value)}</b><em>${esc(note)}</em></span></button>`).join("")}</div><article class="module strategy-table"><div class="module-title">${esc(spec.title)} <span>${tr("ПОД УПРАВЛЕНИЕМ","GOVERNED")}</span></div><div class="strategy-head">${spec.columns.map(column=>`<b>${esc(column)}</b>`).join("")}</div>${rows.map((row)=>`<button class="strategy-row" data-route="${esc(row.route)}">${row.cells.map(cell=>`<span>${esc(cell)}</span>`).join("")}</button>`).join("")}</article><aside class="module strategy-side"><div class="module-title">${esc(spec.sideTitle)} <span>${tr("ПОЛИТИКА","POLICY")}</span></div><dl>${spec.side.map(([label,value])=>`<div><dt>${esc(label)}</dt><dd>${esc(value)}</dd></div>`).join("")}</dl><div class="policy-seal"><i></i><span><b>ЛОКАЛЬНЫЙ УПРАВЛЯЕМЫЙ РЕЖИМ</b><small>Без внешнего исполнения и движения средств</small></span></div></aside></section>`;
}
...
function analystModal() {
  ...
  const question=state.analystPending?tr("Ответ принят. Совет развивает вашу мысль, ищет сильные варианты и формирует следующий вопрос…","Answer received. The council is developing your idea, exploring strong alternatives, and preparing the next question…"):(last?.status==="ASKING"?last.nextQuestion:(turns.length?tr("Контур достаточно ясен для следующего внутреннего исследования.","The contour is clear enough for the next internal research stage."):(analysis.criticalQuestions?.[0]??tr("Что из описанных функций уже реально работает, а что пока существует только как идея?","Which described capabilities work today, and which remain ideas?"))));
