...
function researchedProductMarkup(brand,understanding) {
  ...
  const list=(title,items)=>items?.length?`<section><h3>${title}</h3>${items.map((item)=>`<p>• ${esc(item)}</p>`).join("")}</section>`:"";
  const semantic=analysis?`<h2>${esc(analysis.productName)}</h2><p class="analysis-summary">${esc(analysis.oneLineSummary)}</p><div class="semantic-grid">${list(tr("ДЛЯ КОГО И ЗАЧЕМ","WHO IT SERVES"),analysis.customerSegments)}${list(tr("ЦЕННОСТЬ ПРОДУКТА","PRODUCT VALUE"),analysis.valuePropositions)}${list(tr("ВОЗМОЖНОСТИ","CAPABILITIES"),analysis.productCapabilities)}${list(tr("РИСКИ И ПРОБЕЛЫ","RISKS AND GAPS"),analysis.risks)}${list(tr("ЧТО СИСТЕМА ИССЛЕДУЕТ ДАЛЬШЕ","NEXT INTERNAL RESEARCH"),analysis.recommendedNextResearch)}${list(tr("НУЖНО УТОЧНИТЬ","NEEDS CLARIFICATION"),analysis.criticalQuestions)}</div><details class="raw-observations"><summary>${tr("Показать исходные наблюдения с сайта","Show raw website observations")}</summary>${research.observedClaims.slice(0,10).map((claim)=>`<p>• ${esc(claim)}</p>`).join("")}</details><small class="analysis-meta">AI · ${esc(analysis.model)} · ${esc(analysis.generationId)} · ${analysis.usage.totalTokens??"—"} tokens</small>`:`<h2>${esc(research.observedClaims[0]??understanding.productSummary)}</h2>${list(tr("НАБЛЮДЕНИЯ","OBSERVATIONS"),research.observedClaims.slice(0,8))}${list(tr("ЧТО ЕЩЁ НУЖНО УСТАНОВИТЬ","WHAT REMAINS UNKNOWN"),research.unresolvedQuestions)}`;
  return `<article class="module understanding-review research-result"><div class="module-title">${tr("КАК СИСТЕМА ПОНЯЛА ПРОДУКТ","HOW THE SYSTEM UNDERSTANDS THE PRODUCT")} <span>${tr("НУЖНО ПОДТВЕРЖДЕНИЕ","CONFIRMATION NEEDED")}</span></div>${semantic}<div class="research-sources"><h3>${tr("ИЗУЧЕННЫЕ СТРАНИЦЫ","STUDIED PAGES")}</h3>${research.pages.map((page)=>`<a href="${esc(page.url)}" target="_blank" rel="noreferrer"><b>${esc(page.title)}</b><small>${esc(page.url)}</small></a>`).join("")}</div><div class="modal-actions"><button data-action="edit-brand" data-brand-id="${esc(brand.id)}">${tr("ПОПРАВИТЬ ИЛИ ДОПОЛНИТЬ","CORRECT OR ADD CONTEXT")}</button><button data-action="research-website" data-brand-id="${esc(brand.id)}">${tr("ИЗУЧИТЬ САЙТ ЗАНОВО","RESEARCH WEBSITE AGAIN")}</button><button class="primary" data-action="confirm-understanding" data-brand-id="${esc(brand.id)}">${tr("ДА, ПРОДУКТ ПОНЯТ ВЕРНО","YES, PRODUCT IS UNDERSTOOD")}</button></div></article>`;
}
