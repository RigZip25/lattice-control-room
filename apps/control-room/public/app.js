...
function brandOnboardingMarkup() {
  ...
  let nextAction=councilComplete&&!diagnosis
    ...
    ? `<h3>${tr("Диагноз продукта подтверждён","Product diagnosis is confirmed")}</h3><p>${tr("Следующий этап — сравнить рынки и сформировать последовательность экспансии.","Next, compare markets and shape the expansion sequence.")}</p><button class="primary" data-route="/markets">${tr("СФОРМИРОВАТЬ ТЕЗИС ЭКСПАНСИИ","BUILD EXPANSION THESIS")} →</button>`
...
function render() {
  ...
  const onboardingBrand=["brand-onboarding","brand-diagnosis"].includes(screen.key)?state.brandProfiles.find((item)=>item.id===decodeURIComponent(location.pathname.split("/")[2]??"")):null;
  ...
  document.getElementById("app").innerHTML = `
    ...
    <div class="stats-ribbon"><span>${state.brandProfiles.length} ${tr("БРЕНДОВ","BRANDS")} • ${state.productUnderstandings.filter((item)=>item.websiteResearch?.analysis).length} ${tr("ИЗУЧЕНО","REVIEWED")} • ${state.activationSprints.filter((item)=>item.status==="ACTIVE").length} ${tr("СПРИНТОВ","SPRINTS")} • ${state.addedMarkets.length} ${tr("РЫНКОВ","MARKETS")}</span><b>BUILD 2026.08.28.11 · DRY RUN / ${tr("ФАКТИЧЕСКОЕ СОСТОЯНИЕ","ACTUAL STATE")}</b></div>
    <div class="workspace">
